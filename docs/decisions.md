...
### Skipped source: KidFriendly DC — copy-paste from newsletter instead
**Decided:** 2026-09-09
**Why:** claude.ai reconnaissance (see `docs/scraper-prompt-responses/kidfriendly-dc.md`) surfaced that `kidfriendlydc.com/events/` doesn't serve an events listing at all — it redirects to a stale January 2024 blog post. The site has no dedicated events section, no `/wp-json/tribe/events/v1/events` endpoint (The Events Calendar plugin is not installed), and events aren't structured content: they're embedded inside regular WordPress posts ("Five Things," "Weekend Round-Up") as inline prose like `**[Event Name](url)**  \nWhere: ... \nWhen: ... \nAdmission: ...`. Any automated scraper would be parsing freeform text and would produce mostly-junk data that breaks on every post-format change.
**What replaces the scraper:** Em subscribes to the KidFriendly DC newsletter and, next time she's in a little-dmv session anyway, pastes the newsletter content into chat. Claude extracts the events and files them via `scripts/add_event.py`. This is the "Newsletter-forward" backup path already scoped in the Phase 2 handoffs. Zero automation, zero new habit for Em — just "when you're here, dump what you've got."
**Do not** re-add a `kidfriendly_dc` scraper. Not on the WP REST API (returns blog posts as prose, same parsing problem), not with a browser-based fetcher (Cloudflare / JS execution not the bottleneck — the underlying content is unstructured). The site is not built for scraping and this is not a technology gap that will resolve itself.
